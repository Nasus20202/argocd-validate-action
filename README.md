...
| `skip-files`       | Comma-separated list of files to skip. These files are excluded from discovery/validation, omitted from diff/PR comment output, and removed from `state-dir` before commit when `commit-state: "true"` | No       | ``                         |
...
Use `skip-files` to exclude specific files from processing, hide them from PR diff/comment output, and prevent them from being committed into your manifest state.
